...
@section('content')
    ...
    @endif
    <form action="{{ route('devC-post-update-start', ['id' => $post->id]) }}" method="POST" class="mt-3" enctype="multipart/form-data">
        @csrf
        <div class="row">
            <div class="col-md-6">
                    ...
                    <div class="col-md-12">
                        <div class="image-upload" style="position: relative;">
                            <img class="w-100" style="height: 250px;" id="preview-image"
                                src="<?php if($post->overview_photo != "") { echo $post->overview_photo; } else {echo "https://static.vecteezy.com/system/resources/thumbnails/004/141/669/small/no-photo-or-blank-image-icon-loading-images-or-missing-image-mark-image-not-available-or-image-coming-soon-sign-simple-nature-silhouette-in-frame-isolated-illustration-vector.jpg"; }  ?>"
                                alt="" />
                    ...
                    <div class="col-md-12">
                        ...
                        <select class="js-example-disabled-results form-control" name="category_id">
                            @foreach ($categories as $cate)
                                <option value="{{ $cate->id }}" @if ($cate->id == $post->category_id) selected @endif>{{ $cate->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    @error('category_id')
            ...
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <input type="text" id="title" name="title" class="form-control" placeholder="Tên bài viết" value="{{ $post->title }}">
                    @error('title')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <textarea name="desc_short" class="form-control" id="" cols="30" rows="10">{{ $post->desc_short }}</textarea>
                    @error('desc_short')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="date" class="form-control" name="date_submitted" id="" value="{{ $post->date_submitted }}">
                    ...
                    <div class="col-md-6">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" name="author" id="" readonly value="{{ $post->author }}">
                            @error('author')
                                <div class="text-danger">{{ $message }}</div>
                            @enderror
                        </div>
